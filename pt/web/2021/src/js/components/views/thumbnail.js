...
export var Thumbnail = class extends Scroll {
  ...
  loadImage(page, options={}) {
    ...
    HT.log("// loading:", page.dataset.seq);
  }
  ...
  loadPages() {
    ...
    for(var i = 0; i < tmp.length; i++) {
      ...
      if ( this.isVisible(page) ) {
        ...
        HT.log("@@", seq, page.dataset.lastViewsStarted >= ( 60000 * 2 ));
  ...
  clickHandler(event) {
    ...
    HT.log("-- thumbnail clickHandler", event, event.tagName);
    var check = element.closest('button');
    if ( ! check ) {
      element = element.closest('.page');
      if ( element ) {
        var lastView = this.reader.options.lastView || '1up';
        this.reader.restart({ view: lastView, seq: element.dataset.seq });
      }
    }
  }
