...
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:METS="http://www.loc.gov/METS/" xmlns:PREMIS="http://www.loc.gov/standards/premis" xmlns:exsl="http://exslt.org/common" xmlns:xlink="https://www.w3.org/1999/xlink"
  ...
  <xsl:variable name="gFeaturedCollectionList" select="/MBooksTop/MdpApp/FeaturedCollectionList" />
  ...
  <xsl:variable name="gUsingBookReader">false</xsl:variable>
  ...
  <xsl:template name="build-sidebar-panels">
    ...
    <!-- <xsl:call-template name="build-sidebar-panel">
      ...
      <xsl:with-param name="html">
        <xsl:call-template name="search-in-item-panel" />
      </xsl:with-param>
    </xsl:call-template> -->
  ...
  <xsl:template name="build-sidebar-panel">
    <xsl:param name="id" />
    <xsl:param name="html" />
    <xsl:param name="interactive">true</xsl:param>
    <xsl:param name="aria-expanded">false</xsl:param>

    <xsl:param name="node-set" select="exsl:node-set($html)" />

    <xsl:if test="normalize-space($node-set)">
      <details data-role="accordion" id="{$id}">
        <xsl:if test="$interactive = 'false'">
        ...
        <xsl:apply-templates select="$node-set" mode="copy-filter" />
      </details>
    </xsl:if>
      ...
      <h3>
        ...
        <xsl:if test="$gHtId">
          <p style="margin-bottom: 0.5rem">
            ...
            <xsl:text> </xsl:text>
            <a aria-label="More information" href="https://www.hathitrust.org/accessibility" style="font-size: 1rem"><i class="icomoon icomoon-help"></i></a>
          </p>
        </xsl:if>
        <!-- <p style="font-size: 0.875rem; margin-top: 0">See the <a href="http://www.hathitrust.org/accessibility">HathiTrust Accessibility</a> page for more information.</p> -->
    </div>
  </xsl:template>
  ...
  <xsl:template name="sidebar-about-this-book">
    ...
    <div class="bibLinks panel" id="panel-bib-links">
      ...
      <xsl:if test="//METS:structMap[@TYPE='physical']">
        <p>
          <xsl:value-of select="count(//METS:structMap[@TYPE='physical']/METS:div[@TYPE]/METS:div[@ORDER])" /> page scans
        </p>
      </xsl:if>
      ...
      <xsl:if test="$gHasOcr='YES'">
        <p style="margin-top: 0.25rem; margin-bottom: 0.25rem">
          <a id="ssd-link" href="/cgi/ssd?id={$gHtId}">Text-Only View</a>
        </p>
      </xsl:if>
    ...
    <xsl:if test="$show-download-module = 'true'">
      ...
      <form id="form-download-module" class="form-download-module v2" data-full-pdf-access="{$gFullPdfAccess}" data-format="pdf" autocomplete="off">
        ...
        <fieldset>
          ...
          <xsl:if test="$gFullPdfAccess = 'allow' and $gHasOcr = 'YES'">
            <div class="form-control">
        ...
        <xsl:choose>
          <xsl:when test="$gLoggedIn = 'NO' and $gFullPdfAccessMessage = 'NOT_AFFILIATED'">
            <p class="">
              ...
              <strong><a class="trigger-login" data-close-target=".modal.login" href="{DOWNLOAD_LINK}">Log in</a></strong>
              <xsl:text> to download this item.</xsl:text>
            </p>
            <p>
          ...
          <xsl:when test="$gFullPdfAccessMessage = 'NOT_AVAILABLE'">
            <p>
              <xsl:text>This item cannot be downloaded.</xsl:text>
            </p>
          </xsl:when>
          <xsl:when test="$gFullPdfAccessMessage = 'RESTRICTED_SOURCE'">
  ...
  <xsl:template name="collect-this-book">
    <xsl:variable name="icon">
      <xsl:choose>
        <xsl:when test="count($gCollectionList/Coll) &gt; 0">bi-bookmark-fill</xsl:when>
        <xsl:when test="count($gFeaturedCollectionList/Coll) &gt; 0">bi-bookmark-fill</xsl:when>
        <xsl:otherwise>bi-bookmark</xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <xsl:variable name="class">
      <xsl:choose>
        <xsl:when test="count($gCollectionList/Coll) &gt; 0">listed</xsl:when>
        <xsl:when test="count($gFeaturedCollectionList/Coll) &gt; 0">listed</xsl:when>
    ...
    <div class="collectionLinks panel">
      <h3>
        <xsl:call-template name="build-pt-icon">
          <xsl:with-param name="id" select="$icon" />
          <xsl:with-param name="class" select="$class" />
        </xsl:call-template>
        <span>Collections</span>
      </h3>

      <xsl:if test="count($gFeaturedCollectionList/Coll) &gt; 0">
        <div style="margin-bottom: 1rem;">
          <h4 style="margin-bottom: 0.5rem">In these featured collections:</h4>
          <xsl:if test="false()">
            <h4 style="margin-bottom: 0.5rem">
              <xsl:choose>
                <xsl:when test="count($gCollectionList/Coll) = 1">This item is in this featured collection:</xsl:when>
                <xsl:otherwise>This item is in these featured collections:</xsl:otherwise>
              </xsl:choose>
            </h4>
          </xsl:if>
          <ul class="collection-membership" style="margin-bottom: 1rem">
            <xsl:for-each select="$gFeaturedCollectionList/Coll">
              <li>
                <a href="{Url}"><xsl:value-of select="CollName" /></a>
              </li>
            </xsl:for-each>
          </ul>
        </div>
      ...
      <xsl:if test="$gFinalAccessStatus = 'allow'">
        <p>
          <xsl:element name="a">
            ...
            <xsl:text>Embed this item</xsl:text>
          </xsl:element>
        </p>
      </xsl:if>
    </div>
  ...
  <xsl:template name="CollectionWidgetContainer">
    ...
    <xsl:if test="$gLoggedIn!='YES'">
      <p style="font-size: 90%">
        <strong><a class="PTloginLinkText trigger-login" href="{/MBooksTop/MdpApp/LoginLink}">Log in</a></strong>
    ...
    <div style="margin-bottom: 1rem;">
      ...
      <h4 style="margin-bottom: 0.5rem">In your collections:</h4>
      <xsl:if test="false()">
        <h4 style="margin-bottom: 0.5rem">
          <xsl:text>This item is in your </xsl:text>
          <xsl:choose>
            <xsl:when test="count($gCollectionList/Coll) = 1">collection:</xsl:when>
  ...
  <xsl:template name="BuildAddToCollectionControl">
    <div class="select-collection" style="display: flex; flex-wrap: wrap">
      <label for="PTaddItemSelect" style="flex-basis: 100%"><xsl:text>Add this item to a collection:</xsl:text></label>
      <select id="PTaddItemSelect" class="mdpColSelectMenu" style="width: 75%; margin-right: 0.5rem">
