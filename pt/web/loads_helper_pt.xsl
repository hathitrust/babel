<?xml version="1.0" encoding="utf-8"?>
  ...
  <xsl:variable name="timestamp" select="'?_=1366213683'" />
  <xsl:template name="load_concat_js_file">
    <script type="text/javascript">
        (function() {
            var to_load = [];
            to_load.push("/pt/js/concatenated_common-min.js<xsl:value-of select="$timestamp" />");
    <xsl:if test="//ItemType='volume' and $gSkin='default'">
            to_load.push("/pt/js/concatenated_PAGETURNER_VOLUME_JS-min.js<xsl:value-of select="$timestamp" />");
    </xsl:if>
    <xsl:if test="$gUsingSearch='true'">
            to_load.push("/pt/js/concatenated_PAGETURNER_SEARCH_JS-min.js<xsl:value-of select="$timestamp" />");
    </xsl:if>
    <xsl:if test="$gSkin='mobile' or $gSkin='crms'">
            to_load.push("/pt/js/concatenated_LEGACY_JS-min.js<xsl:value-of select="$timestamp" />");
    </xsl:if>
    <xsl:if test="$gUsingSearch='false' and $gSkin!='default'">
            to_load.push("/pt/js/concatenated_LEGACY_PAGETURNER_JS-min.js<xsl:value-of select="$timestamp" />");
    </xsl:if>
    <xsl:if test="$gUsingSearch='false' and $gItemType = 'volume' and $gUsingBookReader='true' and $gSkin != 'default'">
            to_load.push("/pt/js/concatenated_LEGACY_BOOKREADER_JS-min.js<xsl:value-of select="$timestamp" />");
    </xsl:if>
    <xsl:if test="$gSkin='mobile' and $gItemType='volume' and $gUsingBookReader='true'">
            to_load.push("/pt/js/concatenated_MOBILE_JS-min.js<xsl:value-of select="$timestamp" />");
    </xsl:if>
            head.js.apply(this, to_load);
        })()
    </script>
  </xsl:template> 
