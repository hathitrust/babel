<?xml version="1.0" encoding="utf-8"?>
  ...
  <xsl:variable name="timestamp" select="'?_=1317325766'" />
  ...
  <xsl:template name="load_concat_css_file">
    ...
    <xsl:if test="$gUsingSearch='true'">
        <link rel="stylesheet" type="text/css" href="/pt/concatenated_SEARCH_CSS-min.css{$timestamp}" />
    </xsl:if>
 ...
 <link rel="stylesheet" type="text/css" href="/pt/mobile/mobilept.css{$timestamp}"/>
    ...
    <xsl:if test="$gUsingSearch='true'">
 <link rel="stylesheet" type="text/css" href="/pt/searchresults.css{$timestamp}"/>
    </xsl:if>
</xsl:template>
</xsl:stylesheet>