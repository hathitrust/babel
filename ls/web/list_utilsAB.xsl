...
<!--             <xsl:with-param name="key">
  ...
  <xsl:template name="BuildItemChunk">
    ...
    <xsl:variable name="ClickData">
      <!--XXX to be generated by perl PI handler and put somewhere where we can select it 
	  Consider where to merge global data and local data, i.e. result list=global
	  A or B, id = local
      -->
	  ...
	  <h4>
        ...
        <div class="result-access-link">
		  ...
		  <xsl:attribute name="data_clicklog">
		   <xsl:text>catalog|</xsl:text> <xsl:value-of select="$ClickData"/>
		  </xsl:attribute>
