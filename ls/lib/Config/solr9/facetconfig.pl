...
# Resolve langformat.yaml from this file's own directory (selected by the
# ls_config_dir config key), so the solr9/ copy loads solr9/langformat.yaml
# rather than reaching back into the hardcoded Solr 6 path. $C / MdpConfig is
# not in scope here -- this file is pulled in via require, which does not share
# the caller's lexicals -- so the directory is derived from __FILE__ instead.
my $lang_format_file = dirname(__FILE__) . '/langformat.yaml';
