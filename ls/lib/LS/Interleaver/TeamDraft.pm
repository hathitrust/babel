...
#XXX Move all the other stuff to the base class
#  the __get_interleaved method is the only thing that needs to know what kind of interleaving is to be done

#XXX Also check if all the utility methods can be moved to the base class!

# however the param "start" is only used in Balanced so we need to deal with it

#XXX WARNING  RIGHT now this is just a copy of balanced interleave
