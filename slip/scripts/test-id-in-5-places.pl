...
sub get_slip_solr_result {
    ...
    return ($solr_result, $solr_attr);
}

sub get_slip_rights_result {
    my $id = shift;
    ...
    return ($slip_rights_result, $slip_attr);
}

sub get_rights_current_result {
    my $id = shift;
    ...
    return ($rights_result, $rights_attr);
}

sub get_catalog_result {
    my $id = shift;
...
sub test_ids {
    ...
    if ($num_loaded > 0) {
        ...
        foreach my $id (@$ref_to_arr_of_ids) {
            ...
            my $path_result = get_path_result($id);
            
            my ($solr_result, $solr_attr) = get_slip_solr_result($id);
            
            my ($slip_rights_result, $slip_attr) = get_slip_rights_result($id);

            my ($rights_result, $rights_attr) = get_rights_current_result($id);
            
            my $catalog_result = get_catalog_result($id);
