<?xml version="1.0" encoding="utf-8"?>
  ...
  <xsl:variable name="timestamp" select="'?_=1366217491'" />
  <xsl:template name="load_concat_js_file">
    <script type="text/javascript">
        (function() {
            var to_load = [];
            to_load.push("/mb/js/concatenated_common-min.js<xsl:value-of select="$timestamp" />");
    <xsl:if test="//CurrentCgi/Param[@name='a'] = 'listcs'">
            to_load.push("/mb/js/concatenated_COLLLIST_JS-min.js<xsl:value-of select="$timestamp" />");
    </xsl:if>
