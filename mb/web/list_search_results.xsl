...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- stripped and separated out searchresults stuff to below-->
  ...
  <xsl:template name="SearchResults_status">
    <div class="SearchResults_status">
      ...
      <!-- tbw in suzchaps xxx collection) foobar -->
...
<xsl:template name="SolrError">
  ...
  <xsl:template name="CheckAllIndexed">
    <xsl:variable name="AllIndexedStatus">
      <xsl:value-of select="/MBooksTop/SearchWidget/AllItemsIndexed"/>
    </xsl:variable>
    
    <!--   call javascript function in searchAjaxCommon.js on load (domready)-->
    <xsl:if test="$AllIndexedStatus = 'FALSE'">
      
      <xsl:text disable-output-escaping="yes">&lt;script type="text/javascript"&gt;</xsl:text>
      <xsl:text>YAHOO.util.Event.onDOMReady(notAllIndexed);</xsl:text>
